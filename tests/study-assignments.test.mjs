...
test("defines three paired groups and two reproducible 50-episode judge samples", async () => {
  ...
  for (const group of ["group_a", "group_b", "group_c"]) {
    assert.match(assignments, new RegExp(`value: "${group}"[\\s\\S]*?episodeCount: 100[\\s\\S]*?capacity: 2`));
  }
...
test("enforces assignment visibility, capacity, and separate saved review layers", async () => {
  const [episodes, annotations, users, schema] = await Promise.all([
  ...
  assert.match(users, /two active raters/);
